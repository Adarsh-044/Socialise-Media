...
const server = http.createServer(app);  // For the socket.
const { Server } = require('socket.io');
const io = new Server(server);          // To create connection with express and socket
// io.on('connection', (socket) => {
//     console.log('a user connected');
// });


app.set('view engine', 'ejs');      // Template for the frontend i.e. use of ejs 

const mongoose = require('mongoose');   // To connect with mongoDb
const session = require('express-session'); // To store the previous id for login/home page...      // Here we are not using cookies because -> Cookies are returned and stored in the user's browser, session data is stored on your web server.
const flash = require('connect-flash'); // To show a particular command for some duration 
const MongoDBStore = require('connect-mongo')(session); // To connect mongoDb with session in databases 

const mongoString = `mongodb+srv://admin:socialmedia@cluster0.eckm4pl.mongodb.net/?retryWrites=true&w=majority`;  // (Global host)
//const mongoString="mongodb://localhost:27017/socmed"; (local Host)
...
const passport = require('passport');   // It adds user name and password in the schema.
const LocalStrategy = require('passport-local');
const passportLocalMongoose = require('passport-local-mongoose');

const joi = require('joi'); // To give conditions to the userSchema
const res = require('express/lib/response');
const { KeyObject } = require('crypto');


app.use(express.urlencoded({ extended: true })); // 
app.use(express.json());    // To give parse input 
...
userSchema.plugin(passportLocalMongoose); //will add username,salt,hashed_password into schema 

const store = new MongoDBStore({    // For the cookies. 
    url: mongoString,
    secret: 'randomsecret',
...
const User = mongoose.model('User', userSchema); // users will be added in mongoDb


const sessionConfig = {
    //stores session/cookies in mongodb instead of local memory that vanish if server restart. 
    store,
    secret: 'randomsecret',
    resave: false,
...
const mapdataSchema = new mongoose.Schema({
    ...
    //   }]
    // },
    usermap: {  // It gives user id from the socket id. 
        type: Map,
        of: String
    },
    socketmap: {    // It gives socket it from the user. 
        type: Map,
        of: String
    }
});
...
const chatdataSchema = new mongoose.Schema({    // Used as map like key and value 
    key: String,    // here id1, id2 or id2, id1 are keys
    chats: [{       // chats are displayed or store both users. 
        userid: String, msg: String
    }]
});
...
const checkforOneMapdata = async (req, res, next) => { // Schema have only one object of Mapdata. 
    const data = await Mapdata.find({});
    // console.log('Mapdata-',data);
    if (data.length === 0) {    // No object has been created
...
const validateUser = (req, res, next) => {  // To validate the user and act as a middleware. 
...
app.get('/', isLoggedIn, async (req, res) => {
    const userinfo = await User.findById(req.user._id); // Feature of passport to obtain user detail. 
    const allfriends = userinfo.friends;
    let posts = [];
    for (let friendid of allfriends) {
        const friend = await User.findById(friendid);
        for (let postid of friend.posts) {
            const post = await Post.findById(postid).populate('userid', 'username');  // Populate :: to obtain object of objects. 
            posts.push(post);
        }
    }
...
// Ejs is used to make dynamic html
// Socket.io.
// Passport.
// xmlhttp
// dom 
